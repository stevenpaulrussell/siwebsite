...
class ConnectCases(TestCase):
    ...
    def test_make_a_new_pobox(self):
        ...
        wip = A_Sender.new_card_wip()
        msg = dict(sent_at='sent_at', wip=wip, context='NewSenderFirst', profile_url=A_Sender.profile_url)
        ...
        sender = saveget.get_sender(A_Sender.tel_id)
        print(f'line 167 postoffice tests ===>{sender["sender_moniker"]}')
    ...
    #     self.assertIsNotNone(viewer_data[A.tel_id]['card_id'])    # A's first card was carried over
