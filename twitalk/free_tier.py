...
def mms_to_free_tier(timestamp, from_tel, to_tel, text, image_url, free_tier_morsel):
    ...
    if image_url:  # Either a postcard image or a profile image. Nothing else is valid
        if not text:        
            ...
            filerviews.nq_event(from_tel, to_tel, event_type='profile', profile_url=image_url)
    ...
    if text == 'passkey' and not image_url:         
        ...
        filerviews.nq_event(from_tel, to_tel, event_type='passkey', passkey=passkey, expire=expire)
        from_tel_msg = lines.line(f'Your passkey is "{passkey}", valid in a few minutes, lasting for a day!') 
        return from_tel_msg
    assert(text and not image_url)
    filerviews.nq_event(from_tel, to_tel, event_type='text_was_entered', text=text)
